...
function bigintToUint8Array(bigint, length) {
...
async function schnorrSign(message, privateKey) {
  try {
    console.log('schnorrSign input:', { message, privateKey });
    
    const d = BigInt(`0x${privateKey}`);
    console.log('Private key as BigInt:', d.toString());
    
    const P = pointMultiply(d);
    console.log('Public key point:', P);
    
    // BIP340-compliant nonce generation
    const aux_rand = crypto.getRandomValues(new Uint8Array(32));
    console.log('aux_rand:', bufferToHex(aux_rand));
    
    const t = xor(bigintToUint8Array(d, 32), await taggedHash('BIP0340/aux', aux_rand));
    console.log('t:', bufferToHex(t));
    
    const rand = await taggedHash('BIP0340/nonce', t, bigintToUint8Array(P.x, 32), hexToUint8Array(message));
    console.log('rand:', bufferToHex(rand));
    
    const k = BigInt(`0x${bufferToHex(rand)}`) % CURVE.n;
    console.log('k:', k.toString());
    
    const R = pointMultiply(k);
    console.log('R:', R);
    
    const k_final = (R.y & 1n) ? (CURVE.n - k) : k;
    console.log('k_final:', k_final.toString());
    
    const challenge = await taggedHash('BIP0340/challenge', bigintToUint8Array(R.x, 32), bigintToUint8Array(P.x, 32), hexToUint8Array(message));
    console.log('challenge:', bufferToHex(challenge));
    
    const e = BigInt(`0x${bufferToHex(challenge)}`) % CURVE.n;
    console.log('e:', e.toString());
    
    const s = (k_final + e * d) % CURVE.n;
    console.log('s:', s.toString());
    
    const signature = concatUint8Arrays(bigintToUint8Array(R.x, 32), bigintToUint8Array(s, 32));
    console.log('Final signature:', bufferToHex(signature));
