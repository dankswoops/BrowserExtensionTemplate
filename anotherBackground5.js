...
async function taggedHash(tag, ...messages) {
  ...
  return sha256(concatUint8Arrays(tagHash, tagHash, ...messages));
}
...
async function schnorrSign(messageHash, privateKey) {
  try {
    ...
    const rand = await taggedHash('BIP0340/nonce', t, bigintToUint8Array(P.x, 32), messageHash);
    ...
    const e = BigInt(`0x${bufferToHex(await taggedHash('BIP0340/challenge', bigintToUint8Array(R.x, 32), bigintToUint8Array(P.x, 32), messageHash))}`) % CURVE.n;
...
async function signEvent(event) {
  ...
  try {
    ...
    event.id = bufferToHex(eventHash);

    // Sign the event
    const signatureBytes = await schnorrSign(eventHash, SECKEY);
